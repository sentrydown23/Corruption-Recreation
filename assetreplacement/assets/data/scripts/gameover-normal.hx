...
function create(event) {
    ...
    bgSprite.setGraphicSize(Std.int(FlxG.width * 2.2), Std.int(FlxG.height * 2.2));
    bgSprite.updateHitbox();
    bgSprite.screenCenter();
    bgSprite.scrollFactor.set(0.2, 0.2);
    add(bgSprite);

    lowHealthSprite = new FlxSprite(0, 0).loadGraphic(Paths.image("lowhealth"));
    lowHealthSprite.setGraphicSize(Std.int(FlxG.width * 2.2), Std.int(FlxG.height * 2.2));
    lowHealthSprite.updateHitbox();
    lowHealthSprite.screenCenter();
    lowHealthSprite.scrollFactor.set(0.2, 0.2);
...
function update(elapsed:Float) {
    if (bfSprite != null) {
        ...
        bfSprite.x = ((FlxG.width - bfSprite.width) / 2) + shakeX;
        bfSprite.y = ((FlxG.height - bfSprite.height) / 2) + 150 + shakeY;
    }
    ...
    if (zoomDone && !isEnding) {
        swayTime += elapsed * swaySpeed;
        FlxG.camera.scroll.x = Math.sin(swayTime * 0.8) * 35;
        FlxG.camera.scroll.y = Math.cos(swayTime * 0.6) * 20;
    }
    ...
    if (deathMusic != null && deathMusic.playing && !isEnding) {
        ...
        if (curBeat > lastBeat) {
            ...
            if (zoomDone && curBeat % 4 == 0) {
                ...
                FlxTween.tween(FlxG.camera, {zoom: 0.75}, 2.5, {ease: FlxEase.sineOut});
            }
        }
    }

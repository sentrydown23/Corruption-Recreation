...
class TreeMenuScreen extends FlxSpriteGroup {
	...
	**/
	...
	dynamic function updateItem(object:FlxSprite, itemHeight:Float, centerY:Float, lerpRatio:Float) {
	    ...
	    // Centers the item horizontally relative to the sub-menu screen's position
	    var targetX:Float = x + (FlxG.width - object.width) * 0.5;
	    object.x = CoolUtil.fpsLerp(object.x, targetX, lerpRatio);
	}
